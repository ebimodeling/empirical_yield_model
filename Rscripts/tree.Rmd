...
trainingSet <- as.data.frame(subset(join, select = c("irrigated" , "genus",
...
NBset <- trainingSet[sample(nrow(trainingSet)),]

NBset$irrigated <- as.factor(NBset$irrigated)
NBset$genus <- as.factor(NBset$genus)

NBset$yield <- discretize(NBset$yield, method="frequency", categories = 5, labels = NULL, ordered=FALSE, onlycuts=FALSE)
NBset$yield <- as.factor(NBset$yield)

trainingNB <- NBset[1:600,]
testingNB <- NBset[601:653,]

model <- naiveBayes(yield~., data = trainingNB)

pred <- predict(model, newdata = testingNB)
table(pred, testingNB$yield)
...
library(FSelector)
nbset$yieldn <- NULL 
cfs(yield~.,data=nbset)
chi.squared(yield~.,data=nbset)
information.gain(yield~.,data=nbset)
```
...
nbset <- as.data.frame(nbset)
nbset <- cbind(nbset[1:13],nbset[14]) 
nbset <- nbset[sample(nrow(nbset)),]
pred <- knn(train=nbset[1:600,1:13], test=nbset[601:653,1:13], cl=nbset[1:600,14], k = 1, l = 0, prob = FALSE, use.all = TRUE)

table(pred,nbset[601:653,14])
...
quantile(trainingSet$yield, probs = c(1:9/10))

quantile(trainingSet$yield, probs = c(1:4/5))
